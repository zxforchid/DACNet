...
class DACNet(nn.Module):
    def __init__(self):
        ...
        #stage 6
        self.encoder6_1 = BasicBlock(512,512)
        self.encoder6_2 = BasicBlock(512,512)
        self.encoder6_3 = BasicBlock(512,512)
    ...
    def forward(self,x):
        ...
        ## -------------Encoder1-------------
        hx = self.prerelu(self.prebn(self.preconv(hx)))
        ...
        hx = self.encoder5_1(hx)
        hx = self.encoder5_2(hx)
        h5 = self.encoder5_3(hx)

        hx = self.pool5(h5) 

        hx = self.encoder6_1(hx)
        hx = self.encoder6_2(hx)
        h6 = self.encoder6_3(hx)

        ## -------------Encoder2-------------
        hx1 = self.pooling2(hx1) 
        hx1 = self.prerelu(self.prebn(self.preconv(hx1)))

        h1_1 = self.encoder1(hx1) 
        h1_2 = self.encoder2(h1_1) 
        h1_3 = self.encoder3(h1_2) 
        h1_4 = self.encoder4(h1_3) 

        hx1 = self.pool4(h1_4) 

        hx1 = self.encoder5_1(hx1)
        hx1 = self.encoder5_2(hx1)
        h1_5 = self.encoder5_3(hx1) 
        
        ## -------------Encoder3-------------
        hx2 = self.pooling4(hx2) 
        hx2 = self.prerelu(self.prebn(self.preconv(hx2)))

        h2_1 = self.encoder1(hx2) 
        h2_2 = self.encoder2(h2_1) 
        h2_3 = self.encoder3(h2_2) 
        h2_4 = self.encoder4(h2_3) 
